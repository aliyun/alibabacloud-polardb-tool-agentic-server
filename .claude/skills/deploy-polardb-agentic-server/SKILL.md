...
**Current v0.0.8 onboarding upload contract.** The default deployment is
released `v0.0.8`. This flow exposes only
...
- Docker pulls the `0.0.8` image by default and fails closed if it is unavailable. Use an approved fully qualified `PAS_IMAGE`, or explicitly set `PAS_ALLOW_LOCAL_BUILD=1` to force a local build of the checked-out `PAS_REF` without pulling the default image.
