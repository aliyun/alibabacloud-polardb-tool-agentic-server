...
`pas database migrate` 会向标准错误输出结构化进度记录，使 init container 和
one-shot Pod 日志能够显示当前阶段。记录包含内置与实际要求的 head、发布分类、
数据库连接状态、是否尝试过 Schema 变更、脱敏错误码以及下一步运维动作，但不会
包含数据库 URL 或凭证。manifest 校验失败时，日志会明确说明尚未建立数据库连接，
也没有尝试修改 Schema。
