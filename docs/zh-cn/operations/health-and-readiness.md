...
服务就绪与 Schema 迁移成功相互关联，但不是同一个信号。应用启动会在路由和
Worker 激活前执行只读兼容性检查。托管升级在独立 one-shot Pod 中执行变更，并
要求其 `migrate -> check -> migrate -> check` 命令成功退出，之后才检查目标
应用 Pod。
...
应对持续就绪失败、重启循环、迁移执行器失败、数据库连接耗尽、供应失败和连续
