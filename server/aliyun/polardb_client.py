...
class MockPolarDBClient(PolarDBClient):
    ...
    async def create_agentic_db(self, settings: Mapping[str, str]) -> dict:
        ...
        result: dict = {"cluster_id": cluster_id}
        if settings.get("AgenticDbType") == "dedicated":
            agentic_id = settings.get("AgenticDbClusterId") or (
                "pagc-mock-" + "".join(
                    secrets.choice(string.ascii_lowercase + string.digits) for _ in range(8)
                )
            )
            result["AgenticDbClusterId"] = agentic_id
            if "AgenticDbClusterDescription" in settings:
                result["AgenticDbClusterDescription"] = settings["AgenticDbClusterDescription"]
        return result
...
async def get_polardb_client_async(session) -> PolarDBClient:
    ...
    if ak and sk:
        ...
        duration = await get_setting(session, "aliyun_sts_duration_seconds") or "3600"
        region = await get_setting(session, "pool_region_id") or "cn-hangzhou"

        raw = f"{mode}:{ak}:{sk}:{role_arn}:{session_name}:{duration}:{region}"
        ...
        if mode == "assume_role":
            ...
            provider = DirectAKProvider(ak=ak, sk=sk, region_id=region)

        _client = AliyunPolarDBClient(provider)
        _client_credential_hash = h
        return _client

    # Branch 2: Env fallback
    from server.config import get_config

    config = get_config()
    env_ak = config.aliyun.access_key_id
    env_sk = config.aliyun.access_key_secret

    if env_ak and env_sk:
        env_region = config.aliyun.region_id
        raw = f"env:{env_ak}:{env_sk}:{env_region}"
        h = hashlib.sha256(raw.encode()).hexdigest()
        ...
        provider = DirectAKProvider(ak=env_ak, sk=env_sk, region_id=env_region)
        _client = AliyunPolarDBClient(provider)
        _client_credential_hash = h
        return _client
