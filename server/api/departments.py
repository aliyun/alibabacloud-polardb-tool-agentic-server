...
from server.core import department_manager
...
async def update_department(
    ...
    if body.max_instances is not None:
        ...
        from server.models.instance import Instance, InstanceType, InstanceStatus
...
class RegisterMultitenantInstanceRequest(BaseModel):
    cluster_id: str
    name: str
    host: str
    port: int = 3306
...
async def register_multitenant_instance(
    department_id: str,
    body: RegisterMultitenantInstanceRequest,
    admin: User = Depends(require_admin),
    session: AsyncSession = Depends(get_session),
):
    from server.core.crypto import encrypt
    from server.models import (
        Instance, InstanceType, InstanceStatus,
        DBAccount, AccountType, DepartmentInstanceBinding,
    )

    dept = await department_manager.get_department(session, department_id)
    if dept is None:
        raise HTTPException(404, "Department not found")

    inst = Instance(
        cluster_id=body.cluster_id, name=body.name,
        type=InstanceType.MULTITENANT, status=InstanceStatus.ACTIVE,
        host=body.host, port=body.port, region=body.region,
    )
    session.add(inst)
    await session.flush()
...
async def get_multitenant_instance(
    ...
    from server.models import DepartmentInstanceBinding, Instance, InstanceType
    ...
    result = (await session.execute(
        ...
        .where(
            ...
            Instance.type == InstanceType.MULTITENANT,
        )
        .limit(1)
    )).one_or_none()
    if result is None:
