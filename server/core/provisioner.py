...
async def _preflight_check(session: AsyncSession) -> dict | None:
    ...
    from server.core.settings_manager import get_setting_raw

    errors: list[str] = []
    ak = await get_setting_raw(session, "aliyun_access_key_id")
    sk = await get_setting_raw(session, "aliyun_access_key_secret")
    if not ak or not sk:
        from server.config import get_config

        config = get_config()
        if not config.aliyun.access_key_id or not config.aliyun.access_key_secret:
            errors.append("Cloud credentials not configured (neither DB settings nor env)")
    vpc = await get_setting(session, "pool_vpc_id")
    vswitch = await get_setting(session, "pool_vswitch_id")
    if not vpc or not vswitch:
...
async def startup_recovery_sweep(
    ...
    async with session_factory() as session:
        ...
        timeout = int(
            await get_setting(session, "provisioning_poll_timeout_seconds", default="600") or "600"
        )
        threshold = timeout * 2
        ...
        pool_creating = (
            await session.execute(
                select(Instance).where(
                    Instance.status == InstanceStatus.POOL_CREATING,
        ...
        for inst in pool_creating:
            try:
                ...
                if attr["status"] == "Running":
                    inst.status = InstanceStatus.POOLED
                    logger.info(
                        "recovered POOL_CREATING → POOLED",
                        extra={"metric": "provisioning.recovered", "cluster_id": inst.cluster_id},
                    )
                else:
                    inst.status = InstanceStatus.FAILED
                    logger.warning(
                        "recovered POOL_CREATING → FAILED",
