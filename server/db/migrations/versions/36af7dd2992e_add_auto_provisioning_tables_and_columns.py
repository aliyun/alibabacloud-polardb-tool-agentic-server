...
def upgrade() -> None:
    ...
    with op.batch_alter_table('instances', schema=None) as batch_op:
        batch_op.add_column(sa.Column('provisioning_step', sa.Enum('PENDING', 'CLUSTER_READY', 'PASSWORD_STORED', 'ACCOUNT_CREATED', 'DATABASE_CREATED', 'ENDPOINT_RESOLVED', 'BOUND', 'DONE', name='provisioningstep'), nullable=True))
        batch_op.add_column(sa.Column('quota_held', sa.Boolean(), server_default='0', nullable=False))

    with op.batch_alter_table('users', schema=None) as batch_op:
        batch_op.add_column(sa.Column('provisioning_mode', sa.Enum('DEDICATED', 'MULTITENANT', name='provisioningmode'), nullable=True))
    ...
    for key, sd in SETTINGS_SCHEMA.items():
        op.execute(settings_table.insert().values(
            id=generate_uuid(), key=key, value=sd.default, description=sd.description,
