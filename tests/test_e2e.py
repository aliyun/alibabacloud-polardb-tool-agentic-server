...
class TestE2EFlow:
    ...
    async def test_complete_admin_flow(self, client, setup, encryption_key):
        ...
        # Step 6: List registered physical instances via the admin API. The
        # database-instance Tool family is MCP-only and has no REST mirror.
        resp = await client.get("/api/instances", headers=admin_headers)
        assert resp.status_code == 200
        instances = resp.json()["items"]
