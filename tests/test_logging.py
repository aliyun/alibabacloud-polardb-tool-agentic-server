...
class TestGenerateRequestID:
    ...
    @pytest.mark.parametrize(
        "invalid",
        ["", "x" * 129, "contains spaces", "包含非ASCII", "line\nbreak"],
    )
    def test_normalize_replaces_invalid_values(self, invalid):
        normalized = normalize_request_id(invalid)
        assert normalized != invalid
        assert re.fullmatch(r"[A-Za-z0-9._:-]{1,128}", normalized)

    def test_normalize_preserves_valid_value(self):
        assert normalize_request_id("trace.valid:123") == "trace.valid:123"
