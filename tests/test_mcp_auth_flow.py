...
from server.models import Agent, AgentAPIToken, AuthProvider, Base, User, UserRole
...
def clean():
    ...
    config_module._config = AppConfig(
        server={"public_base_url": "http://localhost:18760"}
    )
    init_test_jwt_keys()
...
async def setup_data(test_engine, encryption_key):
    ...
    async with engine_mod._session_factory() as session:
        ...
        agent = Agent(name="test-agent", created_by=admin.id)
        session.add_all([admin, agent])
        await session.flush()
        agent_token = f"pas_agent_{secrets.token_urlsafe(32)}"
        session.add(
            AgentAPIToken(
                agent_id=agent.id,
                token_prefix=agent_token[:32],
...
class TestMCPAuthFlowBuiltin:
    ...
    async def test_agent_token_can_initialize_without_oauth_flow(
        ...
        response = await client.post(
            ...
            headers={
                **MCP_HEADERS,
                "Authorization": f"Bearer {setup_data['agent_token']}",
            },
        )
