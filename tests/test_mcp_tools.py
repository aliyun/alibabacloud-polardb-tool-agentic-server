...
from server.models import (
    Base, User, Instance, UserInstanceBinding, DBAccount, AuthProvider, UserRole, InstanceType, InstanceStatus, Permission,
)
...
class TestListInstances:
    async def test_list_accessible_instances(self, client, auth_headers, setup_data):
        resp = await client.get("/mcp/rest/list_instances", headers=auth_headers)
        assert resp.status_code == 200
        instances = resp.json()["instances"]
        assert len(instances) >= 1
        assert instances[0]["cluster_id"] == "pc-test-001"
...
class TestRunSQL:
    ...
    async def test_run_sql_returns_mcp_error_for_gateway_exception(self, client, auth_headers, setup_data):
        gateway = SimpleNamespace(execute=AsyncMock(side_effect=RuntimeError("connection lost")))
        ...
        assert data["isError"] is True
        payload = json.loads(data["content"][0]["text"])
        assert payload == {"error": "SQL_ERROR", "message": "connection lost"}
    ...
    async with engine_mod._session_factory() as session:
        ...
        for inst in [personal, shared]:
            db_acct = DBAccount(
                ...
                account_name=f"pas_{inst.cluster_id[:8]}",
                account_password_enc=enc_pw,
            )
            session.add(db_acct)
            await session.flush()
            binding = UserInstanceBinding(
                user_id=user.id, instance_id=inst.id,
                db_account_id=db_acct.id, permission=Permission.READWRITE,
            )
            session.add(binding)
    ...
    async def test_set_inaccessible_instance(
        ...
        async with engine_mod._session_factory() as session:
            other = Instance(
                ...
                type=InstanceType.SHARED, host="127.0.0.1", port=3308,
                status=InstanceStatus.ACTIVE,
            )
            session.add(other)
            await session.commit()
