...
from server.models import (
    Base, User, Instance, UserInstanceBinding, DBAccount,
    AuthProvider, UserRole, InstanceType, InstanceStatus, Permission,
)
...
def auth_headers(setup_data):
    token = _create_mcp_token(setup_data["admin"].id)
...
class TestMCPTransportTools:
    ...
    async def test_run_sql_without_branch_keeps_legacy_tool_result(self, client, auth_headers, setup_data):
        session_id = await self._initialize(client, auth_headers)
        headers = {**MCP_HEADERS, **auth_headers}
        if session_id:
            headers["mcp-session-id"] = session_id
    ...
    async def test_tools_call_list_instances(self, client, auth_headers):
        session_id = await self._initialize(client, auth_headers)
        headers = {**MCP_HEADERS, **auth_headers}
        if session_id:
            headers["mcp-session-id"] = session_id
...
class TestLegacyRESTEndpoints:
    async def test_rest_endpoints_still_work(self, client, rest_auth_headers):
