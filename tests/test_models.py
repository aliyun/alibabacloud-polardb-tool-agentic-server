...
from server.models import (
    ...
    UserInstanceBinding, DepartmentInstanceBinding, DBAccount, AuditLog,
    AuthProvider, UserRole, UserStatus, InstanceType, InstanceStatus,
    Permission, AccountType, AuditStatus,
...
class TestInstanceModel:
    ...
    async def test_unique_cluster_id(self, session: AsyncSession):
        i1 = Instance(cluster_id="pc-dup", name="A", type=InstanceType.SHARED)
        i2 = Instance(cluster_id="pc-dup", name="B", type=InstanceType.SHARED)
        session.add(i1)
        await session.commit()
        session.add(i2)
        with pytest.raises(Exception):
...
class TestBindingModels:
    ...
    async def test_user_instance_binding(self, session: AsyncSession):
        ...
        inst = Instance(cluster_id="pc-003", name="I3", type=InstanceType.SHARED)
    ...
    async def test_department_instance_binding(self, session: AsyncSession):
        ...
        inst = Instance(cluster_id="pc-004", name="I4", type=InstanceType.SHARED)
...
class TestDBAccountModel:
    async def test_create_db_account(self, session: AsyncSession):
        user = User(external_id="u4", display_name="U4")
        inst = Instance(cluster_id="pc-005", name="I5", type=InstanceType.SHARED)
        session.add_all([user, inst])
        await session.commit()
        ...
        session.add(account)
        await session.commit()
        assert account.account_name == "pas_u4"

    async def test_unique_instance_user_account(self, session: AsyncSession):
        user = User(external_id="u5", display_name="U5")
        inst = Instance(cluster_id="pc-006", name="I6", type=InstanceType.SHARED)
        session.add_all([user, inst])
        await session.commit()

        a1 = DBAccount(instance_id=inst.id, user_id=user.id, account_name="a1", account_password_enc="pw1")
        session.add(a1)
        await session.commit()
        a2 = DBAccount(instance_id=inst.id, user_id=user.id, account_name="a2", account_password_enc="pw2")
        session.add(a2)
        with pytest.raises(Exception):
            await session.commit()
...
class TestUserDefaultInstance:
    ...
    async def test_set_default_instance_id(self, session: AsyncSession):
        ...
        inst = Instance(cluster_id="pc-def-001", name="Default Inst", type=InstanceType.SHARED)
        session.add_all([user, inst])
        await session.commit()
    ...
    async def test_fk_set_null_on_instance_delete(self, session: AsyncSession):
        ...
        inst = Instance(cluster_id="pc-def-002", name="Deletable", type=InstanceType.PERSONAL)
        session.add_all([user, inst])
        await session.commit()
