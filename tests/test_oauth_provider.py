...
class TestCodeExchange:
    async def test_load_valid_code(self, provider, session_factory):
        ...
        assert loaded.subject == "user:user-1"
...
class TestAccessToken:
    ...
    async def test_load_active_agent_token(self, provider, session_factory):
        async with session_factory() as session:
            ...
            session.add(agent)
            await session.flush()
            row, plaintext = await create_test_agent_token(session, agent.id)
            await session.commit()
    ...
    async def test_unknown_agent_token_is_rejected(self, provider):
