...
from server.models import (
    Base, Instance, InstanceType, InstanceStatus, User, AuthProvider,
    UserInstanceBinding, DBAccount, AccountType,
)
...
async def user_and_instance(session):
    ...
    inst = Instance(
        ...
        type=InstanceType.PERSONAL, status=InstanceStatus.CREATING,
...
class TestCompleteProvisioning:
    ...
    async def test_happy_path_from_cluster_ready(
        ...
        async with session_factory() as s:
            ...
            # Check DBAccount was created
            db_acc = (await s.execute(
                select(DBAccount).where(DBAccount.instance_id == inst.id)
            )).scalar_one()
            assert db_acc.account_name == "agentic"
    ...
    async def test_resume_from_password_stored(
        ...
        # Pre-create DBAccount (simulates PASSWORD_STORED step completed before crash)
        async with session_factory() as s:
            i = await s.get(Instance, inst.id)
            ...
            db_acc = DBAccount(
                ...
                account_name="agentic", account_password_enc=encrypt(pw),
