...
from server.models.binding import UserDepartment, UserInstanceBindingCapability
...
def _make_provisioning_credential(
    ...
    return InstanceCredential(
        ...
        password_ciphertext=encrypt(generate_db_password()),
            ...
            credential = (
                await s.execute(
                    select(InstanceCredential).where(
                        InstanceCredential.instance_id == inst_id
                    )
                )
            ).scalar_one()
...
class TestPasswordStoredState:
    async def test_creates_account(self, session_factory, seeded):
        ...
        async with session_factory() as s:
            ...
            s.add(_make_provisioning_credential(inst_id, user_id))
            await s.commit()
            ctx = _make_ctx(
                s, inst, session_factory, MockPolarDBClient(), user_id
            )
...
class TestEndpointResolvedState:
    async def test_creates_binding_and_default(self, session_factory, seeded):
        ...
        async with session_factory() as s:
            ...
            assert binding.permission == Permission.READWRITE
    ...
    async def test_existing_disabled_admin_binding_preserves_authorization(
        self, session_factory, seeded
    ):
        user_id, inst_id = seeded
        async with session_factory() as s:
            inst = await s.get(Instance, inst_id)
            inst.provisioning_step = ProvisioningStep.ENDPOINT_RESOLVED
            credential = _make_provisioning_credential(inst_id, user_id)
