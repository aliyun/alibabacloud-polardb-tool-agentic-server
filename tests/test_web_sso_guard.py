...
from server.config import load_config, reset_config
...
def test_web_sso_guard_config_from_env(monkeypatch):
    ...
    config = load_config()
    assert config.auth.web_sso_guard.enabled is True
    assert config.auth.web_sso_guard.session_ttl_hours == 12
...
class TestAppIntegration:
    def test_guard_not_registered_when_disabled(self, monkeypatch):
        monkeypatch.setenv("PAS_SERVER_DEV_MODE", "true")
        monkeypatch.setenv("PAS_AUTH_WEB_SSO_GUARD_ENABLED", "false")
        reset_config()
    ...
    def test_guard_registered_when_enabled_with_oidc(self, monkeypatch):
        monkeypatch.setenv("PAS_SERVER_DEV_MODE", "true")
    ...
    def test_guard_warns_when_enabled_without_oidc(self, monkeypatch):
        monkeypatch.setenv("PAS_SERVER_DEV_MODE", "true")
        monkeypatch.setenv("PAS_AUTH_MODE", "builtin")
        monkeypatch.setenv("PAS_AUTH_WEB_SSO_GUARD_ENABLED", "true")
        reset_config()
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            create_app()
            guard_warnings = [x for x in w if "SSO Guard is disabled" in str(x.message)]
...
class TestE2EGuardFlow:
    ...
    @pytest.mark.asyncio
    async def test_protected_path_redirects_to_sso(self):
        from server.app import create_app
        app = create_app()
        ...
        async with httpx.AsyncClient(
            ...
            assert resp.status_code == 302
            assert "login.example.com" in resp.headers.get("location", "")
